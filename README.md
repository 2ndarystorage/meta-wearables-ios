...
- iOS SDK (Swift Package) plus sample apps demonstrating Meta Wearables Device Access Toolkit integration with Meta AI glasses (device registration, streaming video, photo capture, connection states).
- Sample apps include CameraAccess/QuickCapture/QuickCaptureLite for wearables streaming and PhotoTextScan for photo-library OCR using Apple Vision.

## How to Use

- Not verified: Open a sample `*.xcodeproj` under `samples/` in Xcode (15+), set signing/bundle ID, and build/run on an iOS 17+ device.
- Not verified: For glasses streaming samples, enable Developer Mode in the Meta AI app and update `MetaAppID`/`ClientToken` in `Info.plist`; the simulator is for UI checks only.
- Not verified: For PhotoTextScan, grant photo library access and run on device or simulator (simulator requires adding photos).

## Completion Status

- Partial (demo-quality): The SDK is labeled developer preview and the repository primarily ships sample apps and integration guidance rather than production-hardened software.

## Program Summary

- iOS SDK (Swift Package) plus sample apps demonstrating Meta Wearables Device Access Toolkit integration with Meta AI glasses (registration, connection states, streaming video, and photo capture).
- Samples include CameraAccess/QuickCapture (full streaming UI), QuickCaptureLite (minimal streaming + filters/overlay), and PhotoTextScan (photo-library OCR without glasses).

## How to Use

- Not verified: Open a sample `*.xcodeproj` under `samples/` in Xcode, set signing/bundle ID, and build/run on an iOS 17+ device.
- Not verified: For glasses streaming samples, enable Developer Mode in the Meta AI app and update `MetaAppID`/`ClientToken` in `Info.plist` as required by the sample README.
- Not verified: PhotoTextScan can run on device or simulator with photo library access (simulator requires adding photos).

## Completion Status

- Partial (developer preview): The SDK is labeled developer preview and the repo focuses on sample apps and integration flows rather than production-hardened applications.

## Program Summary

- iOS SDK (Swift Package) and sample apps for the Meta Wearables Device Access Toolkit, enabling iOS apps to connect to Meta AI glasses for streaming video and photo capture.
- Samples include CameraAccess/QuickCapture (full streaming UI), QuickCaptureLite (minimal streaming with filters and a basic face-detection overlay), and PhotoTextScan (photo-library OCR sample).

## How to Use

- Not verified: Add the SDK via Swift Package Manager as described above.
- Not verified: Open a sample `*.xcodeproj` under `samples/` in Xcode 15+, update bundle ID plus `MetaAppID`/`ClientToken` in `Info.plist`, and run on an iOS 17+ device; simulator is for UI checks only for streaming samples.

## Completion Status

- Partial (developer preview): The README labels the toolkit as developer preview and the repository is primarily SDK + sample applications, not production-hardened software.

## Program Summary

- iOS SDK (Swift Package) plus sample apps for Meta Wearables Device Access Toolkit, focused on connecting to Meta AI glasses for streaming video and photo capture.
- Samples include CameraAccess/QuickCapture/QuickCaptureLite for glasses streaming flows and PhotoTextScan for photo-library OCR using Apple Vision.

## How to Use

- Not verified: Add the SDK via Swift Package Manager as described above or open a sample `*.xcodeproj` under `samples/` in Xcode.
- Not verified: For glasses streaming samples, enable Developer Mode in the Meta AI app and set `MetaAppID`/`ClientToken` in `Info.plist`, then build/run on an iOS 17+ device.
- Not verified: PhotoTextScan runs on device or simulator with photo library access (simulator requires adding photos).
